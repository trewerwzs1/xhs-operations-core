...
class BridgeServer:
    def __init__(self) -> None:
        ...
        self._extension_build_id = ""
        self._extension_instance_id = ""
    ...
    async def _handle_extension(self, ws: ServerConnection, handshake: dict) -> None:
        ...
        if previous is not None and previous is not ws:
            # An unpacked-extension reload can overlap the old and new service
            # workers for a fraction of a second.  Fail any in-flight command
            # closed and retire the old socket before publishing the new owner.
            # The old handler's ``finally`` must not clear the replacement.
            for future in self._pending.values():
                if not future.done():
                    future.set_exception(ConnectionError("Extension reloaded during command"))
            self._pending.clear()
            try:
                await previous.close(code=1000, reason="superseded by reloaded extension")
            except Exception:
                logger.debug("关闭旧 Extension 连接失败", exc_info=True)

        logger.info("Extension 已连接")
        self._extension_ws = ws
        self._extension_build_id = str(handshake.get("build_id", ""))
        self._extension_instance_id = str(handshake.get("extension_instance_id", ""))
        try:
            async for raw in ws:
                try:
                    msg = json.loads(raw)
            ...
            if self._extension_ws is ws:
                ...
                logger.info("旧 Extension 连接已退出；当前重载连接保持有效")
        ...
        if msg.get("method") == "ping_server":
            await ws.send(json.dumps({
                "result": {
                    ...
                    "extension_build_id": self._extension_build_id,
